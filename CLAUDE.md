...
- **First real playtest (`./gradlew runClient`) found and fixed a fragmentation bug**: one contiguous forest was splitting into multiple differently-named entities, causing the notification to flicker between names. `NaturalRegionDiscovery` now merges a newly-discovered patch into an adjacent same-type entity instead of always creating a new one. See [context/decisions.md](context/decisions.md), "Fixed natural-region fragmentation" entry. **Still needs a second playtest to confirm the fix actually works** — not yet done as of this entry.
- See [context/classes/](context/classes/) for per-class reference.

Next: confirm the fragmentation fix in-game (walk a forest/desert boundary repeatedly, watch for repeated flicker), then likely either start on a second mod in the suite that actually consumes this API, or revisit one of the deferred items (EntityType extensibility, spatial index persistence, an `-api` Gradle module split) when something concrete needs it.
