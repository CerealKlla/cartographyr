...
- Demo entry notification — `CartographyrMod`'s `PlayerTickEvent.Post` listener checks the player's position 5x/second, debounces via two-consecutive-matching-checks before announcing (filters jagged-biome-border flicker without feeling laggy), triggers natural-region discovery if nothing's known there. Session-only (in-memory, not persisted) — a demo feature only, not real player-knowledge tracking (that's Lyfe's job now, see below).
- **Presentation changed 2026-09-24** (see [context/decisions.md](context/decisions.md)): the old "You have entered <name>" chat message was replaced with a persistent top-right "Location: <name>" HUD overlay (`.network.LocationPayload`, `.client.ClientLocationState`/`LocationOverlay`), per user request. Also added a login-sync fix (previously a reconnect-without-moving left the overlay blank indefinitely) and caught a real registration pitfall (`RegisterPayloadHandlersEvent` shares one global registry per modid — the payload can only be registered once, not once per dist-gated class, or the client crashes on boot with "already registered"). Boot-verified clean on both `runServer` (schema-only path, no client class loaded) and `runClient` (full path with the overlay).
